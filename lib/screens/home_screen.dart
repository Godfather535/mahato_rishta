...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _checkUserVipStatus() async {
    ...
    InAppPurchase.instance
        .restorePurchases()
        .then((dynamic pastPurchases) {
      // UserModel().setUserVip(); ///TODO: User is set as VIP from this method call
  ...
  void _handlePurchaseUpdates() {
    ...
    _inAppPurchaseStream = InAppPurchase
      ...
      for (var purchase in purchases) {
        ...
        switch (purchase.status) {
          ...
          case PurchaseStatus.canceled:
            //TODO: purchase status canceled
            break;
          case PurchaseStatus.restored:
            //TODO: purchase status restored
            break;
    ...
    return StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData) {
            ...
            /// Get total counter to alert user
            final total = snapshot.data!.docs
                .where((doc) => doc[MESSAGE_READ] == false)
                .toList()
                .length;
            if (total == 0) return icon;
            return NotificationCounter(icon: icon, counter: total);
