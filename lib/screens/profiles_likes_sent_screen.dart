...
class ProfileLikesByUserScreen extends StatefulWidget {
  const ProfileLikesByUserScreen({Key? key}) : super(key: key);

  @override
  _ProfileLikesByUserScreenState createState() =>
      _ProfileLikesByUserScreenState();
}

class _ProfileLikesByUserScreenState extends State<ProfileLikesByUserScreen> {
  // Variables
  ...
  void _loadMoreUsersListener() async {
    _gridViewController.addListener(() {
      if (_gridViewController.position.pixels ==
        ...
        if (_loadMore) {
          _likesApi
              .getLikedByyMeUsers(loadMore: true, userLastDoc: _userLastDoc)
              .then((users) {
            /// Update users list
            if (users.isNotEmpty) {
  ...
  void initState() {
    super.initState();
    _likesApi.getLikedByyMeUsers().then((users) {
      // Check result
      if (users.isNotEmpty) {
        if (mounted) {
          setState(() {
        ...
        body: Column(
          children: [
            ...
            BuildTitle(
              ...
              title: _i18n.translate("users_who_liked_by_you"),
            ),
                      ...
                      onTap: () {
                        ...
                        if (UserModel().userIsVip) {
                          ...
                          _visitsApi.visitUserProfile(
                            ...
                            nMessage: _i18n.translate(
                                "visited_your_profile_click_and_see"),
                          );
                        } else {
                          /// Show VIP dialog
                          showDialog(
